...
function buildCinematicCanvasFilter(aspect: JobAspect): string {
  ...
  return [
    ...
    // Cinematic vignette + subtle moving grain over the whole canvas.
    // These native filters replace the old `geq` dust/shadow layers, which
    // evaluated trig expressions per-pixel, per-frame, single-threaded over the
    // full 1080x1920 canvas (x3) on every segment — the main cost driver.
    // `vignette` keeps the edge-darkening look; a light `noise` grain gives a
    // living, film-grain "dust" feel across the whole frame at near-zero cost.
    // Grain strength kept low (alls=4) so it stays under the segment maxrate
    // cap instead of starving the picture; bump it for a coarser look.
    `[composed]format=yuv420p,vignette=angle=PI/4.2,noise=alls=4,setsar=1[vout]`,
  ].join(";");
}
