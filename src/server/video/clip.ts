 ...
 */
async function analyzeLoudSegments(
    videoPath: string
): Promise<LoudSegment[]> {
...
function pickClipWindows(
    ...
    const candidates = loudSegments.filter(
        (seg) => seg.duration >= clipDuration
    );
    ...
    for (const seg of candidates) {
        ...
        if (clipStart + clipDuration > seg.end)
            clipStart = seg.end - clipDuration;
...
export async function createClipsFromVideoUsingRanges(
    ...
    for (const range of ranges) {
        let start = Number.isFinite(range.start) ? Math.max(0, range.start) : 0;
        let end = Number.isFinite(range.end) ? range.end : start + 1;

        if (end <= start) {
            end = start + 1; // at least 1 second
        }
