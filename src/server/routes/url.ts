...
export async function registerUrlRoute(app: FastifyInstance) {
    app.post("/api/url", async (req, reply) => {
        ...
        const clipDurationSec =
            typeof body.clipDurationSec === "number" && body.clipDurationSec > 0 ? body.clipDurationSec : 30;

        const maxClips =
            typeof body.maxClips === "number" && body.maxClips > 0 ? body.maxClips : 3;

        const captionsEnabled =
            typeof body.captionsEnabled === "boolean" ? body.captionsEnabled : true;
