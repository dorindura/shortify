...
export async function enforceJobLimits(
  ...
  if (requested.jobGoal === "summary" && requested.summaryTargetSec > 60) {
    return { ok: false as const, reason: "Free plan: summary max 60s." };
  }

  if (requested.jobGoal === "shorts") {
  ...
  if ((count ?? 0) >= FREE_MAX_TOTAL_JOBS) {
    return {
      ok: false as const,
      reason: "Free plan: max 10 jobs total. Upgrade to Pro to create more.",
    };
  }
