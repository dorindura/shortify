        ...
        <div className="mt-3 rounded-xl border border-emerald-500/20 bg-slate-950/60 px-3 py-3">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="w-full text-[10px] text-slate-500">
              ...
              reels. Each selected angle becomes one job.
            </div>
          </div>
          ...
          {angleCandidates.length > 0 && (
            <div className="mt-3 space-y-2">
              ...
              <button
                type="button"
                onClick={onCreateFromAngles}
                disabled={loading || selectedAngleIdx.length === 0}
                className="mt-1 inline-flex w-full items-center justify-center rounded-xl bg-emerald-500 px-4 py-2 text-xs font-semibold text-slate-950 transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {loading
                  ? "Creating..."
                  : `Create ${selectedAngleIdx.length} reel${
                      selectedAngleIdx.length === 1 ? "" : "s"
                    } from selected`}
              </button>
            </div>
          )}
