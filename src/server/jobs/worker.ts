...
import { createClipsFromVideo, createClipsFromVideoUsingRanges } from "@server/video/clip";
import { generateSubtitlesForClips } from "@server/video/caption";
import { renderShortsWithSubtitles } from "@server/video/render";
import { analyzeTranscriptForClips, ClipCandidate } from "@server/video/scoring";
...
export async function processJob(jobId: string) {
  ...
  try {
    ...
    await cleanupLocalJobArtifacts({
      ...
      audioPaths,
      subtitlePaths,
      extraPaths: [...videos, ...(thumbs ?? []).filter(Boolean)],
    });
