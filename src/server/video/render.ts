...
import type { JobAspect, CaptionStyle } from "@lib/jobsStore";
...
function buildCropXExprForSegments(segments: SmartCropSegment[]): string {
    ...
    const lerp = (a: string, b: string, u: string) =>
        `(${a}+(${b}-${a})*${u})`;
...
export async function renderShortsWithSubtitles(
    ...
    for (let i = 0; i < clips.length; i++) {
        ...
        const subsPath = subtitleFiles[i]
            ? path.resolve(subtitleFiles[i])
            : null;
