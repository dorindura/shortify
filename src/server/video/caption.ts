...
function buildAssHeader(style: CaptionStyle, fontName = DEFAULT_FONT): string {
  const styleLines = buildAssStyleLines(style, fontName);
...
function buildAssStyleLines(style: CaptionStyle, fontName: string): string[] {
  const defaultStyle = buildDefaultStyleLine(style, fontName);
  const centerStyle = buildCenterWordStyleLine(style, fontName);
  return [defaultStyle, centerStyle];
}

function buildDefaultStyleLine(style: CaptionStyle, fontName: string): string {
  const base = {
    Name: "Default",
    Fontname: fontName,
    Fontsize: 56,
...
function buildCenterWordStyleLine(style: CaptionStyle, fontName: string): string {
  if (style === "boldYellow") {
    return (
      "Style: " +
      [
